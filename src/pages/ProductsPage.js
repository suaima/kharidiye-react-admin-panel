...
function descendingComparator(a, b, productsBy) {
  if (b[productsBy] < a[productsBy]) {
    return -1;
  }
  if (b[productsBy] > a[productsBy]) {
    return 1;
...
function getComparator(products, productsBy) {
  return products === 'desc'
    ? (a, b) => descendingComparator(a, b, productsBy)
    : (a, b) => -descendingComparator(a, b, productsBy);
}
...
function applySortFilter(array, comparator, query) {
  ...
  stabilizedThis.sort((a, b) => {
    const products = comparator(a[0], b[0]);
...
export default function ProductsPage() {
  ...
  const [productsBy, setProductsBy] = useState('name');
  ...
  const handleRequestSort = (event, property) => {
    const isAsc = productsBy === property && products === 'asc';
    setProducts(isAsc ? 'desc' : 'asc');
    setProductsBy(property);
  };
  ...
  const filteredUsers = applySortFilter(DATALIST, getComparator(products, productsBy), filterName);
  ...
  return (
    <>
      <Helmet>
        <title> Products | Kharidiya </title>
      </Helmet>
